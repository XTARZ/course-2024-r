...
all_transcripts_covid_related_middle <- all_transcripts_covid_related
...
topic_prop_mid_1 <- topic_prop
topic_prop_long <-
  topic_prop %>% pivot_longer(
    cols = -"doc_id",
    names_to = "topics",
    values_to = "p"
  )
topic_prop_long_mid_1 <- topic_prop_long
topic_prop_long <-
  topic_prop_long %>% left_join(agg_topics_map, by = "topics")
topic_prop_long <-
  topic_prop_long %>% replace_na(list(type = "others", name = "others"))
topic_prop_long_agg <-
  topic_prop_long %>% group_by(doc_id, name)

topic_prop_long_agg_mid_1 <- topic_prop_long_agg

topic_prop_long_agg <-
  topic_prop_long_agg %>% summarise(p = sum(p))

topic_prop_long_agg_mid_2 <- topic_prop_long_agg
...
all_transcripts_covid_related_f <-
  all_transcripts_covid_related_copy %>% filter(length >= vocab_lower)
...
for (new_topic_name in new_topic_names) {
  example_topic <-
    ...
    top_n(
      n = num_of_examples,
      wt = get(new_topic_name)
    ) %>%
    select(text, !!new_topic_name)
  example_topic$topic <- new_topic_name
  example_topic <-
    example_topic %>%
    rename(agg_prop = !!new_topic_name) %>%
    arrange(topic)
...
if (OUTPUT_TOPDOC) {
  output_df %>%
    select(topic, example_num, text, agg_prop) %>%
    mutate(agg_prop = round(agg_prop, 2)) %>%
    openxlsx::write.xlsx(
      file = file.path(
        "output",
        "stm",
...
topic_prob <-
  ...
  t() %>%
  as_tibble() %>%
  slice(rep(1:n(), each = length(stm_model$vocab)))
topic_prob_mid_1 <- topic_prob
topic_prob %>% dim()
...
word_topic_agg <-
  ((as.matrix(P_W_and_T_agg) / as.matrix(topic_prob_agg)) * sum(stm_model$settings$dim$wcounts$x)) %>%
  as_tibble()
...
word_df <-
  ...
  mutate(
    ...
    w_color = sample.int(7, n(), replace = TRUE)
  ) # 30% of vertical
...
plot_wc <-
  ...
  theme_bw() +
  labs(x = paste0("Topic ", topic), y = NULL) +
...
plot_topic <- function(topic,
  ...
  word_df %>% ggplot(aes(
    ...
    theme_minimal() +
    labs(x = paste0(topic_name), y = NULL) +
...
if (OUTPUT_WORDCLOUD) {
  ...
  #   height = 4
  # )
  do.call("plot_grid", c(lapply(1:length(exposure_topics), plot_topic, 30, c(2.5, 6.5)), nrow = 2))
  ggsave(
    file.path("output", "stm", "exp", "agg", model_name, "exposure_1.png"),
    width = 8,
    height = 4
  ...
  do.call("plot_grid", c(lapply((length(exposure_topics) + 1):(length(exposure_topics) + length(response_topics)), plot_topic, 30, c(2.5, 6.5)), nrow = 1))
  ggsave(
...
prop_weighted <- topic_prop %>%
  group_by(call_title_date) %>%
