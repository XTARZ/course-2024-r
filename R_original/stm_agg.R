...
topic_prob <-
  topic_prob %>% t %>% as_tibble() %>% slice(rep(1:n(), each = length(stm_model$vocab)))
topic_prob %>% dim
...
word_topic_agg <-
  ((as.matrix(P_W_and_T_agg) / as.matrix(topic_prob_agg)) * sum(stm_model$settings$dim$wcounts$x)) %>% as_tibble()
